...
def create_model(heat_recovery=False, calc_boiler_eff=False, capital_fs=False):
    """
    Create simple rankine cycle model.
    """
    ...
    # create arcs
    m.fs.boiler_to_turbine = Arc(
        source=m.fs.boiler.outlet, destination=m.fs.turbine.inlet
    )

    if heat_recovery:
        m.fs.turbine_to_precondenser = Arc(
            source=m.fs.turbine.outlet, destination=m.fs.pre_condenser.inlet
        )
        m.fs.precondenser_to_condenser = Arc(
            source=m.fs.pre_condenser.outlet, destination=m.fs.condenser.inlet
        )
        ...
        m.fs.turbine_to_condenser = Arc(
            source=m.fs.turbine.outlet, destination=m.fs.condenser.inlet
        )

    m.fs.condenser_to_pump = Arc(
        source=m.fs.condenser.outlet, destination=m.fs.bfw_pump.inlet
    )
    ...
    m.fs.gross_cycle_power_output = Expression(
        expr=(
            -m.fs.turbine.work_mechanical[0] - m.fs.bfw_pump.work_mechanical[0]
        )
    )
    ...
    if calc_boiler_eff:
        ...
        m.fs.boiler_eff = Expression(
            expr=0.2143
            * (m.fs.net_cycle_power_output * 1e-6 / m.fs.net_power_max)
            + 0.7357
        )

    #  cycle efficiency
    m.fs.cycle_efficiency = Expression(
        expr=m.fs.net_cycle_power_output
...
def initialize_model(m, outlvl=idaeslog.INFO):
    """
    Initialize simple rankine cycle model.
    """
...
def generate_report(m, unit_model_report=True):
    """
    Print report model results for a single rankine cycle.
    User may optioanlly choose to print results for individual
    unit operations.
    """
    # Print reports
...
def set_inputs(m, bfw_pressure=24.23e6, bfw_flow=10000):
    """
    Set operating conditions for the various units in the
    rankine cycle model.
    """
    # Main steam pressure
...
def close_flowsheet_loop(m):
    ...
    boiler. When the pressure and enthalpy arcs are enabled, the
    bfw_pump spec for deltaP and the inlet enth_mol for the boiler need
    to be unfixed.

    Returns: m: model object after closing the loop
    """
...
def add_capital_cost(m):
    """
    Add capital cost expressions. Leverages costing correlations from
    the IDAES costing library. Note that the capital cost correlations
    are all based on the boiler feed water flowrate.

    Returns: m: model object after adding capital cost correlations
    """
    ...
    get_PP_costing(
        m.fs.turbine, turb_power_account, m.fs.turbine_power_mw, "kW", 2
    )
    ...
    get_PP_costing(
        m.fs.bfw_pump, fwh_power_account, m.fs.bfw_lb_hr, "lb/hr", 2
    )
...
def add_operating_cost(m, include_cooling_cost=True):
    ...
    m.fs.cw_flow = Expression(
        expr=-m.fs.condenser.heat_duty[0]
        * 0.018
        * 0.26417
        * 3600
    ...
    m.fs.coal_hhv = Param(
        initialize=27113, doc="Higher heating value of coal as received kJ/kg"
    )

    # cost of coal (Reference - NETL baseline report rev #4)
    m.fs.coal_cost = Param(
        initialize=51.96, doc="$ per ton of Illinois no. 6 coal"
    )
    ...
    m.fs.coal_flow = Expression(
        expr=(
            (m.fs.boiler.heat_duty[0] / m.fs.boiler_eff * 3600)
    ...
    if include_cooling_cost:
        ...
        # Expression for total operating cost
        m.fs.operating_cost = Expression(
            expr=m.fs.total_coal_cost, doc="Total operating cost in $/hr"
        )
